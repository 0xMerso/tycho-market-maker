...
pub fn find_optimal_swap_amount(
    ...
    let initial_spot_price = protosim.spot_price(
        if base_is_token0 { selling_token } else { buying_token },
        if base_is_token0 { buying_token } else { selling_token },
    ).map_err(|e| format!("Failed to get initial spot price: {:?}", e))?;
    
    
    // First check if max amount can reach the target
    let max_post_swap_price = calculate_post_swap_price(
        protosim, selling_token, buying_token, max_amount, 
        selling_pow, buying_pow, base_is_token0
    )?;
    simulation_count += 1;
    
    let (_, max_execution_price) = calculate_swap_output(
        protosim, selling_token, buying_token, max_amount, 
        selling_pow, buying_pow, base_is_token0
    )?;
    simulation_count += 1;
    ...
    let overshoots = if initial_spot_price < reference_price {
        ...
        if max_post_swap_price > reference_price {
            tracing::info!("Max amount overshoots target: Pool {:.2} → {:.2} (target: {:.2}). Binary search will find exact amount.", 
                         initial_spot_price, max_post_swap_price, reference_price);
            true
        } else {
            false
        }
    ...
    for _iteration in 0..OPTI_MAX_ITERATIONS {
        ...
        let post_swap_price = calculate_post_swap_price(
            protosim, selling_token, buying_token, mid, 
            selling_pow, buying_pow, base_is_token0
        )?;
        simulation_count += 1;

        // Also get execution price for reporting
        let (_, execution_price) = calculate_swap_output(
            protosim, selling_token, buying_token, mid, 
            selling_pow, buying_pow, base_is_token0
        )?;
        simulation_count += 1;
...
fn calculate_post_swap_price(
    ...
    let post_swap_price = result.new_state.spot_price(
        if base_is_token0 { selling_token } else { buying_token },
        if base_is_token0 { buying_token } else { selling_token },
...
fn calculate_swap_output(
    ...
    if amount_normalized < f64::EPSILON {
        ...
        let spot_price = protosim.spot_price(
            if base_is_token0 { selling_token } else { buying_token },
            if base_is_token0 { buying_token } else { selling_token },
        ).unwrap_or(0.0);
